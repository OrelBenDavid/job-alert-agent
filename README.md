...
בלי למחוק היסטוריה. `/list` מציג את כל החברות וסטטוס שלהן.
...
- `mobileye`, `wiz` — פרופילי API (Lever/Greenhouse), נבנו לפי מה שכבר
  אומת בעבר. **`health.expected_min_jobs` עדיין placeholder** - הרשת
  בסביבת הבנייה לא אפשרה קריאה חיה. יש להריץ `--seed` פעם אחת אמיתית
  ואז לעדכן את הערך בפרופיל.
