...
def build_lever(row):
    ...
    verify_raw.csv's `host` column."""
    slug = row["id"]
    eu = row.get("host") == "lever_eu"
...
def build_record(row):
    ...
    record = {
        ...
        "notes": (
            ...
            "verification (tier %s in bot_shortlist.csv). Everything not stated "
            "here - fetch_type, the field map, pagination, the Israel filter and "
            "detail_fetch - is inherited from profiles/_platforms/%s.json. Live "
            "at import: %s postings on the board, %s Israel-relevant."
            % (row["tier"], row["ats"], row["jobs"], row["israel_jobs"])),
    }
    return record, None
...
def main():
    ...
    print()
    print("rows in verify_raw.csv      %d" % len(rows))
    print("  dead (skipped)            %d" % len(dead))
